...
const FilteredJobsButtons = () => {
  ...
  const handleEdit = (url, e) => {
    e.stopPropagation(); // Prevents panel click from triggering navigation
    navigate(url); // Uses navigate function for navigation
  };
      ...
      <Stack spacing={20} direction="column">
        {jobs.map((job, index) => (
          <Panel
            key={index}
            bordered
            onClick={(e) => handleEdit(job.url, e)}
            style={{
              display: 'flex',
              alignItems: 'center',
          ...
          >
            <div style={{ flex: 1 }}>
              <h4 style={{ margin: 0, color: '#333' }}>{job.heading}</h4>
              <p style={{ margin: 0, color: '#555' }}>Amount: {job.amount}</p>
            </div>
